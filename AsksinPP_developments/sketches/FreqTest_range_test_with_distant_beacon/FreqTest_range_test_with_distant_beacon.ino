...
#endif
#if defined ARCH_AVR_1284P
// we use a ATMega 1284p
...
#define BEACON_MSG_INTERVAL 10        // value of MSG_INTERVAL of the beacon sketch 'Range_test_beacon.ino'

// see https://github.com/AskSinPP/asksinpp.de/blob/master/Grundlagen/FAQ/Fehlerhafte_CC1101.md
#define RSSI_FROM_SPECIFIC_DISTANT_SENSOR_ONLY // uncomment if you want to see only the RSSI values of s specific distant sensor, e.g. for range tests
//#define ACTIVE_PING
HMID PING_FROM(0x77, 0x77, 0x77);    // from address for status message: e.g. AC powered switch (for option ACTIVE_PING) or of distant sensor (for option RSSI_FROM_SPECIFIC_DISTANT_SENSOR_ONLY)
HMID PING_TO(0x12,0x34,0x56);        // to address for status message / central / CCU
#ifdef ACTIVE_PING
  #define SCANTIME seconds2ticks(5)  // maximal time to wait for a valid message
#elif defined(RSSI_FROM_SPECIFIC_DISTANT_SENSOR_ONLY)
  #define SCANTIME seconds2ticks(BEACON_MSG_INTERVAL+5) // maximal time to wait for a valid message  // adapt on transmit interval of your specific distant sensor: choose transmit interval of distant sensor + 10
#else
...
#ifdef RSSI_FROM_SPECIFIC_DISTANT_SENSOR_ONLY
    if (msg.from() == PING_FROM)
...
#elif defined(RSSI_FROM_SPECIFIC_DISTANT_SENSOR_ONLY)
    DPRINT("Range analysis is enabled: Looking only for messages from a specific distant sensor/beacon with address ");
    PING_FROM.dump(); DPRINTLN(" !");
#else
...
void setup () {
  ...
  DPRINTLN("Range analysis version of FreqTest. Please run a beacon with 'Range_test_beacon.ino' in parallel");
  sdev.init(hal);
  // start sender
