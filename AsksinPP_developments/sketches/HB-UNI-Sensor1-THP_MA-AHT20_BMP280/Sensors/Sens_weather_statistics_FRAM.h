...
template<typename T, typename P> class  weather_statistics {
  ...
  private:
    ...
    struct  nvm_weather_statistics_parameters {                                     // set of statistical parameters that will be stored in a non-volatile memory FRAM
      ...
      uint8_t                             circular_buffer_number;                   // number of circular buffer
  ...
  public:
    weather_statistics(FUJITSU_MB85RS2MTPF_FRAMs* frams, uint32_t start_address, uint8_t cb_number, long CIRCULAR_BUFFER_SIZE, char IDENT_CHAR) {
      ...
      */
      ...
      nvm.restore_count                      = 0;
      nvm.frams                              = frams;
      nvm.circular_buffer_number             = cb_number;
    ...
    bool check_and_copy_struct(nvm_weather_statistics_parameters* FRAM_struct_ptr, nvm_weather_statistics_parameters* RAM_struct_ptr){
      bool success = true;
      nvm_tmp.start_addr                     = nvm.start_addr;
      DPRINTLN(F("struct nvm_weather_statistics_parameters stored in FRAM bank is:"));
      dump_nvm_weather_statistics_parameters(*FRAM_struct_ptr);
      DPRINTLN(F("struct nvm_weather_statistics_parameters stored in RAM bank is:"));
      dump_nvm_weather_statistics_parameters(*RAM_struct_ptr);
      ...
      // check correct identifier string in struct FRAM_struct_ptr
      if (success) {
        success = ( FRAM_struct_ptr->ident_char == RAM_struct_ptr->ident_char );
        status_error_message_1(success, "identifier character" );
        DPRINTLN(FRAM_struct_ptr->ident_char);
      }
      // check correct circular buffer index of struct FRAM_struct_ptr
      if (success) {
        success = ( FRAM_struct_ptr->circular_buffer_number == RAM_struct_ptr->circular_buffer_number );
        status_error_message_1(success, "circular_buffer_number of circular buffer" );
        DDECLN(FRAM_struct_ptr->circular_buffer_number);
      }
      // check correct dimension nof circular buffer in struct FRAM_struct_ptr
      if (success) {
        success = ( FRAM_struct_ptr->BUFSIZE == RAM_struct_ptr->BUFSIZE );
        status_error_message_1(success, "dimension of circular buffer" );
        DDECLN(FRAM_struct_ptr->BUFSIZE);
      }
      // check if struct FRAM_struct_ptr has been saved at least once
      if (success) {
        success =  (FRAM_struct_ptr->save_count > 0 );
        status_error_message_1(success, "parameter save_count" );
        DDECLN(FRAM_struct_ptr->save_count);
      }
      // check if struct FRAM_struct_ptr has been initialized
      if (success) {
        success =  FRAM_struct_ptr->initialized_flag;
        status_error_message_1(success, "parameter initialized_flag" );
        DDECLN(FRAM_struct_ptr->initialized_flag);
      }
      if (success) {
        if (FRAM_struct_ptr->frams != RAM_struct_ptr->frams) {
          DPRINTLN(F("fixing a different frams_ptr"));
          FRAM_struct_ptr->frams = RAM_struct_ptr->frams;
        }
      }
      // copy struct FRAM_struct_ptr to struct RAM_struct_ptr if all checked passed successfully
      if (success) {
        DPRINTLN(F("successfully restored struct nvm_weather_statistics_parameters from FRAM bank"));
        memcpy(RAM_struct_ptr, FRAM_struct_ptr, sizeof(*FRAM_struct_ptr));
        /*DPRINTLN(F("struct nvm_weather_statistics_parameters stored in FRAM bank is:"));
        dump_nvm_weather_statistics_parameters(*FRAM_struct_ptr);
        DPRINTLN(F("struct nvm_weather_statistics_parameters stored in RAM bank is:"));
        dump_nvm_weather_statistics_parameters(*RAM_struct_ptr);*/
        DPRINTLN(F("memcpy done"));
      }
      else {
    ...
    void dump_nvm_weather_statistics_parameters(nvm_weather_statistics_parameters nvm){
      ...
      DPRINT(F("-> circular_buffer_number         : "));
      DDECLN(nvm.circular_buffer_number);
      DPRINT(F("-> start_addr                     : "));
      DHEXLN(nvm.start_addr);
...
#ifdef DEEP_DEBUG
      DPRINTLN(F("read_DP_from_FRAM"));
      nvm.frams->print_FRAM_sizes();
#endif

      DP_FRAM_address = calculate_DP_FRAM_address(index);
#ifdef DEEP_DEBUG
      DPRINTLN(F("calculate_DP_FRAM_address"));
      nvm.frams->print_FRAM_sizes();
#endif
    ...
    bool initialize_circular_buffer( int16_t preset_value10 = 106, bool presetHistoricTemperatures = true ) {
      bool     success;
      ...
      if ( nvm.frams->get_FRAM_start_status_flag() == FRAMS_UNINITIALIZED ) {
        ...
        if (presetHistoricTemperatures) {
          ...
          success= nvm.frams->FRAM_region_flush16(preset_value10, nvm.start_DPs_address, nvm.end_DPs_address);
          nvm.moving_sum                         = (P)(preset_value10 * nvm.BUFSIZE); // preset moving_sum
        }
    ...
    uint32_t modulo_function_for_circular_index (uint32_t circular_index) {
      return (circular_index % nvm.BUFSIZE);
    }
    ...
    uint32_t add_measurement( T new_weather_datapoint ) {
      ...
      nvm.count_index += 1;
      DPRINT(F(".. adding a new measurement value to circular buffer '"));
      DDEC(nvm.circular_buffer_number);
      DPRINTLN(F("'"));
      ...
      nvm.circular_index                         = modulo_function_for_circular_index(nvm.circular_index + 1);
      DPRINT(F("added measurement value as new head : ")); DDECLN(new_weather_datapoint);
    ...
    bool preset_circular_buffer_with_historical_average_temperatures() {
      ...
      while (number_of_presets_to_be_done > 0 ) {
        ...
        j = modulo_function_for_circular_index(j + 1);        // increment index modulo nvm.BUFSIZE
