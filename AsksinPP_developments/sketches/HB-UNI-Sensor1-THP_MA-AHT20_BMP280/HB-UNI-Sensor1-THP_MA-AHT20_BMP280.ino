...
//-------------------------------------------------------------
// the sensor HB-UNI-Sensor1-THP-AHT20_BMP280 requires mandatorily an ATMega1284P MCU, recommended is the PCB HB-UNI-SEN-BATT_ATMega1284P_E07-868MS10_FRAM_FUEL4EP,
...
const struct DeviceInfo PROGMEM devinfo = {
    ...
    { 0xF6, 0x0F },    // Device Model        needs to fit to Addon XML hb-uni-sensor-THP-AHT20_BMP280.xml line 6:
...
public:
    void init(uint8_t msgcnt, int16_t temp, uint16_t my_humidity10, uint16_t airPressure10, uint16_t operatingVoltage1000, int16_t MA_24H_temp100, int16_t MA_7D_temp100, int16_t MA_1M_temp100, int16_t MA_1Y_temp100, bool batLow)
    {
        ...
        DDEC(operatingVoltage1000);DPRINT(F("/"));
        ...
        Message::init(0x19, msgcnt, 0x70, flags, t1, t2); //  length 25 = 0x19 bytes (see also addon hb-ep-devices-addon/CCU_RM/src/addon/firmware/rftypes/hb-uni-sensor-THP-AHT20_BMP280.xml)
...
public:
        ...
        : RegList0<Reg0>(addr)
   {
    ...
    void defaults()  // will be executed when executing a factory reset by pressing the config button >= 6 seconds
    {
        clear();
        ledMode(1);
...
DEFREGISTER(UReg1, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12)
class SensorList1 : public RegList1<UReg1> {
  public:
    ...
    bool bootType (int8_t value) const {
      return
          this->writeRegister(0x0D, (value) & 0xff)
          ;
    }

      int8_t bootType () const {
      return
          (int8_t)(this->readRegister(0x0D, 0))
          ;
    }

    bool presetHistoricTemperatures (int8_t value) const {
      return
          this->writeRegister(0x0E, (value) & 0xff)
          ;
    }

    int8_t presetHistoricTemperatures () const {
      return
          (int8_t)(this->readRegister(0x0E, 0))
          ;
    }

    bool presetTemp10(int32_t value) const {
      return
          this->writeRegister(0x0F, (value >> 24) & 0xff) &&
          this->writeRegister(0x10, (value >> 16) & 0xff) &&
          this->writeRegister(0x11, (value >> 8) & 0xff) &&
          this->writeRegister(0x12, (value) & 0xff)
          ;
    }

    int32_t presetTemp10() const {
      return
          ((int32_t)(this->readRegister(0x0F, 0)) << 24) +
          ((int32_t)(this->readRegister(0x10, 0)) << 16) +
          ((int32_t)(this->readRegister(0x11, 0)) << 8) +
          ((int32_t)(this->readRegister(0x12, 0)))
          ;
    }
...
class WeatherChannel : public Channel<Hal, SensorList1, EmptyList, List4, PEERS_PER_CHANNEL, SensorList0>, public Alarm {
    ...
    bool      regularWakeUp;
    float     temperature_offset, humidity_offset, pressure_offset, presetTemp;
...
public:
...
#ifdef SENSORS_AHT20_BMP280
        AHT20_BMP280.measure(temperature_offset, humidity_offset, pressure_offset);
...
#ifdef SENSORS_AHT20_BMP280
        AHT20_BMP280.init((uint16_t)this->device().getList0().altitude(), temperature_offset, humidity_offset, pressure_offset);
#endif
    ...
    void configChanged() {
      ...
      temperature_offset = (float)this->getList1().tempOffset10()  / 10.0;
      humidity_offset    = (float)this->getList1().humidOffset10() / 10.0;
      pressure_offset    = (float)this->getList1().pressOffset10() / 10.0;
      presetTemp         = (float)this->getList1().presetTemp10()  / 10.0;
      ...
      DPRINT(F("* Preset historic temp       : ")); DDECLN(this->getList1().presetHistoricTemperatures());
      DPRINT(F("* Preset temperatures        : ")); DDECLN(presetTemp);
