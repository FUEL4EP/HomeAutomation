...
// Version 1.1 / USE FOR INITIAL DEBUG PURPOSE ONLY VERSION!!! NORMAL FUNCTION IS NOT GIVEN!!!
// DEBUG rLF version
// AsksinPP Bosch BME680 VOC sensor sketch with auto-compensation of temperature and relative humidity by a Kalman filter
// (C) 26.05.2023 FUEL4EP (Creative Commons)
...
// the sensor HB-UNI-Sensor1-AQ-BME680_KF_rLF_DEBUG requires mandatorily an ATMega1284P MCU (Pro Mini XL - v2 - ATmega 1284p https://www.tindie.com/products/prominimicros/pro-mini-xl-v2-atmega-1284p/)
...
#include "sensors/sens_bme680_KF_rLF_DEBUG.h"
...
const struct DeviceInfo PROGMEM devinfo = {
  {0xf6, 0x0B, 0x01},       // Device ID             // change 0x01 to your sensor's index number, if you use multiple HB-UNI-Sensor1-AQ-BME680_KF_rLF_DEBUG sensors, please increase the index for each sensor by 1
  "BME6800B01",             // Device Serial         // change NAME to your name abbreviation, if you use multiple HB-UNI-Sensor1-AQ-BME680_KF_rLF_DEBUG sensors, please increase the index for each sensor by 1
// 1234567890                                        // device serial must be exactly 10 characters
  {0xf6, 0x0C},             // Device Model Indoor   // needs to fit to Addon XML hb-uni-sensor-AQ-BME680_KF_rLF_DEBUG.xml line 6:
                            //                       // parameter index="10.0" size="2.0" const_value="0xF60C" />
...
class WeatherEventMsg : public Message {
  public:
    void init(uint8_t msgcnt, int16_t temp, uint8_t aq_level, uint16_t aq_state_scaled, int16_t   aq_compensated_gas_res_raw_scaled, int16_t   aq_compensated_gas_res_min_scaled, int16_t   aq_compensated_gas_res_max_scaled, int16_t   aq_alpha_scaled, int16_t   aq_beta_scaled, bool batlow, uint16_t operatingVoltage1000) {
      ...
      pload[0] =   aq_level & 0xff;
      
      pload[1] =   (aq_state_scaled >> 8) & 0xff;
      pload[2] =   (aq_state_scaled >> 0) & 0xff;
      
      pload[3] =  (aq_compensated_gas_res_raw_scaled >> 8) & 0xff;
      pload[4] =  (aq_compensated_gas_res_raw_scaled >> 0) & 0xff;
      
      pload[5] =  (aq_compensated_gas_res_min_scaled >> 8) & 0xff;
      pload[6] =  (aq_compensated_gas_res_min_scaled >> 0) & 0xff;
      
      pload[7] =  (aq_compensated_gas_res_max_scaled >> 8) & 0xff;
      pload[8] =  (aq_compensated_gas_res_max_scaled >> 0) & 0xff;
      
      pload[9] =  (aq_alpha_scaled >> 8) & 0xff;
      pload[10] =  (aq_alpha_scaled >> 0) & 0xff;
      
      pload[11] =  (aq_beta_scaled >> 8) & 0xff;
      pload[12] =  (aq_beta_scaled >> 0) & 0xff;
...
class WeatherChannel : public Channel<Hal, SensorList1, EmptyList, List4, PEERS_PER_CHANNEL, SensorList0>, public Alarm {
    WeatherEventMsg msg;
    Sens_Bme680<BME680_I2C_ADDRESS>   bme680;
    uint16_t            millis;
  ...
  public:
    ...
    virtual void trigger (__attribute__ ((unused)) AlarmClock& clock) {
      ...
      DPRINT(F("corrected T                            = "));  DDECLN(bme680.temperature());
      DPRINT(F("scaled aq state                        = "));  DDECLN(bme680.aq_state_scaled());
      DPRINT(F("battery voltage                        = "));  DDECLN(operatingVoltage1000);

      msg.init( msgcnt, bme680.temperature(), bme680.aq_level(), bme680.aq_state_scaled(), bme680.aq_compensated_gas_res_raw_scaled(), bme680.aq_compensated_gas_res_min_scaled(), bme680.aq_compensated_gas_res_max_scaled(), bme680.aq_alpha_scaled(), bme680.aq_beta_scaled(), device().battery().low(), operatingVoltage1000);
