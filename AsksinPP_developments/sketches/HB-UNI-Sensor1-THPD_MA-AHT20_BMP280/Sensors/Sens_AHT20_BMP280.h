...
public:
  ...
  void init (uint16_t altitude, int8_t tempOffset10, int8_t humidOffset10, int8_t pressOffset10, int8_t bootType, int8_t presetHistoricTemperatures, int16_t presetTemp10) {
    ...
    if (aht20.begin() == false)
    {
      ...
      DPRINTLN(F("AHT20 offsets (all x10):"));
    ...
    if (!status) {
      ...
      DPRINTLN(F("BMP280 offset (x10):"));
      DPRINT(F("-P    : ")); DDECLN(pressOffset10);

      _bootType                    = bootType;
      _presetHistoricTemperatures  = presetHistoricTemperatures;
      _presetTemp10                = presetTemp10;
      DPRINT(F("boot type                     :")); DDECLN(bootType);
      DPRINT(F("preset historic temperatures  :")); DDECLN(presetHistoricTemperatures);
      DPRINTLN(F("preset temp (x10):"));
      DPRINT(F("-PRE_T                        : ")); DDECLN(presetTemp10);
    }
  ...
  if ( success ) {
    ...
    success &= _1Y_moving_average_temperature_statistics.initialize_circular_buffer(_presetTemp10, _presetHistoricTemperatures);
    success &= _1M_moving_average_temperature_statistics.initialize_circular_buffer(_presetTemp10, _presetHistoricTemperatures);
    success &= _7D_moving_average_temperature_statistics.initialize_circular_buffer(_presetTemp10, _presetHistoricTemperatures);
    success &= _24h_moving_average_temperature_statistics.initialize_circular_buffer(_presetTemp10, _presetHistoricTemperatures);
  ...
  _present = _present_BMP280 && _present_AHT20 && success;
  }
  ...
  void measure () {
    ...
    if (_present == true) {
      ...
      if ( !success ) {
        DPRINTLN(F("ERROR: Cannot enter low ppower mode of FRAMs!"));
      }
